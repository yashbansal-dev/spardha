...
export default function SpardhaLoader({ onComplete, className }: SpardhaLoaderProps) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setProgress((prev) => {
                ...
                // Linear progress calculation to hit exactly 3 seconds
                // Adding a tiny bit of randomness for "digital" feel but keeping it tight
                const randomFactor = (Math.random() * 0.5) + 0.75;
                return Math.min(prev + (INCREMENT * randomFactor), 100);
            });
        }, INTERVAL_MS);
    ...
    // Text Scramble Effect tied to Progress
    useEffect(() => {
        // Calculate how many letters should be "revealed" based on progress
        const revealedCount = Math.floor((progress / 100) * targetWord.length);
        ...
        // Status Text Updates
        if (progress < 30) setLoadingText("INITIALIZING");
        else if (progress < 60) setLoadingText("LOADING ASSETS");
        else if (progress < 90) setLoadingText("CONNECTING...");
